...
test("service snapshot: 同项目同 PGID 的多 PID、多端口与 wrapper 聚合为一个服务", () => {
  ...
  const snapshot = buildServiceSnapshot([first, second], [reg("web", 3000)]);
  assert.equal(snapshot.services.length, 1);
  assert.deepEqual(snapshot.services[0].ports, [3000, 3001]);
  assert.deepEqual(snapshot.services[0].listenerPids, [101, 102]);
  assert.deepEqual(snapshot.services[0].wrapperPids, [100]);
  assert.equal(snapshot.services[0].confidence, "corroborated");
});

test("service snapshot: 一个 PID 的多个端口只生成一个服务", () => {
  const snapshot = buildServiceSnapshot([proc(101, [3000, 3001], 100)], []);
  assert.equal(snapshot.services.length, 1);
  assert.deepEqual(snapshot.services[0].ports, [3000, 3001]);
});

test("service snapshot: 同一 claim 的 service id 不随监听端口变化", () => {
  const first = buildServiceSnapshot([proc(101, [3000], 100)], [reg("web", 3000)]).services[0];
  const second = buildServiceSnapshot([proc(101, [3001], 100)], [reg("web", 3000)]).services[0];
  assert.equal(first.id, second.id);
  assert.equal(second.claims[0].relation, "drift");
});

test("service snapshot: 同项目不同 PGID 不自动合并", () => {
  const snapshot = buildServiceSnapshot([proc(101, [3000], 100), proc(201, [4000], 200)], []);
...
test("service snapshot: 同项目共享端口的多个 PID 即使 PGID 不同也聚合", () => {
  const snapshot = buildServiceSnapshot([proc(101, [3000], 100), proc(201, [3000], 200)], []);
  assert.equal(snapshot.services.length, 1);
  assert.deepEqual(snapshot.services[0].listenerPids, [101, 201]);
});
...
test("service snapshot: runId 与 PGID 匹配时规划 managed-run，停止前仍需 marker 复核", () => {
  const snapshot = buildServiceSnapshot([proc(101, [3000], 100)], [
    reg("web", 3000, { runPid: 100, runId: "run-1" }),
  ]);
  assert.equal(snapshot.services[0].confidence, "corroborated");
  assert.equal(snapshot.services[0].attachment, "managed");
  assert.equal(snapshot.services[0].stopMode, "managed-run");
});
...
test("service snapshot: 同项目存在多个 live service 时 reserved claim 不猜归属", () => {
  ...
  assert.equal(snapshot.services.find((service) => service.activity === "reserved")?.ports[0], 5173);
});

test("service snapshot: run-d claim 的 runPid 已死时服务状态为 dead", () => {
  const snapshot = buildServiceSnapshot([], [reg("dead", 3000, { runPid: 999999, runId: "dead-run" })]);
  assert.equal(snapshot.services[0].activity, "dead");
  assert.deepEqual(snapshot.services[0].warnings, ["managed-run-dead"]);
});
...
test("service snapshot: live 项目与 claim 项目冲突时阻止 stop", () => {
  ...
  assert.equal(snapshot.services[0].confidence, "conflict");
  assert.equal(snapshot.services[0].stopMode, "blocked");
  assert.deepEqual(snapshot.services[0].warnings, ["claim-conflict"]);
});
...
test("service snapshot: 无项目 detached listener 保持 unknown 并报警", () => {
  ...
  assert.equal(snapshot.services[0].confidence, "unknown");
  assert.deepEqual(snapshot.services[0].warnings, ["unknown-ownership", "detached-unverified"]);
});
